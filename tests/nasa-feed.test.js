...
function test () {
  return axios.get(`https://api.nasa.gov/neo/rest/v1/feed?start_date=${getDate()}&api_key=DEMO_KEY`)
    .then(({ data }) => {
      ...
      try {
        ...
        throw new Error('Unexpected data structure, was looking for :root.near_earth_objects[array]');
      }
      ...
      if (typeof first.is_potentially_hazardous_asteroid === 'undefined') {
        throw new Error('Missing key "is_potentially_hazardous_asteroid" from first data point, presuming remaining data is wrong.');
      }
    });
}
